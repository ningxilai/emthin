...
- **smithay `X11Wm` selection 钩子**（`XwmHandler::{new_selection, send_selection, cleared_selection}`） — 一份协议参考，emthin 自身已不再使用该路径，X ↔ Wayland 翻译现在由外部 [`xwayland-satellite`](https://github.com/Supreeeme/xwayland-satellite) 进程承担。
