# emskin-launch: full XDG Desktop Entry parser

## Purpose

Current `emskin-launch.el` extracts only `Name` and `Exec` from `.desktop`
...
The `emskin--app-list` cache stores these plists instead of cons cells.
`completing-read` displays `:name` (optionally annotated with `:categories`
or `:comment` via `annotation-function`).

## Parser rewrite (`emskin--desktop-parse`)
...
| `Exec` | `:exec` | raw — substitution happens in `emskin-open-app` |
...
## Scan rewrite (`emskin--desktop-scan`)
...
- Keep the `emskin--exec-wayland-p` filter (ldd check) unchanged
- Result is a list of plists, stored in `emskin--app-list`

## Exec field-code substitution (`emskin--substitute-field-codes`)

Applied in `emskin-open-app` after selecting the app and action (if any).
...
All code stays in `emskin-launch.el`. Expected growth:

| Section | Current lines | Target lines |
|---|---|---|
| Requires + caches | 24 | 20 |
| Parser (`emskin--desktop-parse`) | 29 | 120 |
| Scanner (`emskin--desktop-scan`) | 16 | 25 |
| Field-code substitution | 0 | 50 |
| Launcher (`emskin-open-app`) | 20 | 35 |
| **Total** | **109** | **~300** |

## Testing

Add test file `elisp/tests/emskin-launch-tests.el` with fixture
