...
//!
//! Every client reaches emthin via Wayland — X clients come through
...
impl KeyboardFocusTarget {
    ...
    fn inner(&self) -> &dyn KeyboardTarget<EmthinState> {
        match self {
            Self::Window(w) => w
                .toplevel()
                .expect("X clients reach emthin as Wayland toplevels via xwayland-satellite")
                .wl_surface(),
...
impl KeyboardTarget<EmthinState> for KeyboardFocusTarget {
    fn enter(
        &self,
    ...
    fn leave(&self, seat: &Seat<EmthinState>, data: &mut EmthinState, serial: Serial) {
