...
pub struct ImeBridge {
    ...
    /// Caret area in **emthin-winit-local** coords (already translated
...
impl ImeBridge {
    pub fn new(dh: &DisplayHandle) -> Self {
        ...
        let _ = TextInputManagerState::new::<EmthinState>(dh);
        Self {
    ...
    pub fn poll_tip_freshness(&mut self, seat: &Seat<EmthinState>, apps: &AppManager) {
...
/// Translate a client-surface-local rect into emthin-winit-local
/// `(position, size)`.
...
smithay::delegate_text_input_manager!(EmthinState);
