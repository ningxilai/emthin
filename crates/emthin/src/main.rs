...
fn register_dbus_listen_source(
    event_loop: &smithay::reexports::calloop::EventLoop<EmthinState>,
    state: &EmthinState,
) -> Result<(), Box<dyn std::error::Error>> {
    use smithay::reexports::calloop::{generic::Generic, Interest, Mode, PostAction};
    use std::os::unix::io::FromRawFd;
...
fn handle_dbus_accept(state: &mut EmthinState) {
    loop {
...
/// state is freed.
fn register_dbus_connection(state: &mut EmthinState, accepted: emthin_dbus::ConnAccepted) {
    use emthin_dbus::PumpOutcome;
    use smithay::reexports::calloop::{generic::Generic, Interest, Mode, PostAction};
    use std::os::unix::io::FromRawFd;
...
fn drop_dbus_connection(state: &mut EmthinState, id: emthin_dbus::ConnId, side: DropSide) {
